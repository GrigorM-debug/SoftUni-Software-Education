    ...
    public abstract class DbContext
    {
        ...
        private void MapAllRelations()
        {
            foreach(var dbSetProperty in this.dbSetProperties)
            {
                ...
                mapRelationsGeneric.Invoke(dbSet, new [] {dbSet});
            }
        }
        ...
        public void SaveChanges()
        {
            ...
            foreach (IEnumerable<object> dbSet in dbSets)
            {
                ...
                {
                    int lenght = invalidEntities.Length;
                    string typeName = invalidEntities.GetType().Name;

                    throw new InvalidOperationException(string.Format(Exceptions.InvalidEntity, lenght, typeName));
                }
            }
            ...
            using (new ConnectionManager(this.connection))
            {
                using (var transaction = this.connection.StartTransaction())
                {
                    foreach(IEnumerable dbSet in dbSets)
                    {
                        ...
                        try
                        {
                            persistMethod.Invoke(dbSetType, new object[] {dbSet });
                        }
                ...
                where TEntity : class, new()
        {
            ...
            foreach(var collection in collections)
            {
                ...
                var mapCollectionMethod = typeof(DbContext)
                    .GetMethod("MapCollections", BindingFlags.Instance | BindingFlags.NonPublic)
                    .MakeGenericMethod(entityType, collectionType);
                mapCollectionMethod.Invoke(dbSet, new object[] {dbSet, collection});
            }
        }
        ...
        {
            var tableName = ((TableAttribute)tableType.GetCustomAttribute(typeof(TableAttribute))).Name;
            ...
            where TEntity : class
        {
            ...
            var columns = GetEntityColumnsNames(table);
        ...
        private string[] GetEntityColumnsNames(Type table)
        {
            var tableName = GetTableName(table);
